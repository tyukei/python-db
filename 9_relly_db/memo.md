...
B+Tree 状態 (テーブル: phones):
  Meta Page ID: 13
  Root Page (先頭 64バイト):
00000000000000010000005480049549000000000000008c056274726565948c04506169729493942981947d94288c036b657994430931000000000000000194
```
...
- 9~: 実際のデータ
- 
00000000：4 バイトでノードタイプ（0 = リーフノード）を示す。
00000001：4 バイトでこのノードに1つのキー・値ペアがあることを示す。
00000054：4 バイトでペアデータのサイズが84バイトであることを示す。
80 04
→ pickle ヘッダー。プロトコル 4 を使用している。
95
→ FRAME 命令のオペコード。これに続く8バイトがフレームサイズ。
49 0000000000000008
→ FRAME 命令に続く8バイトのサイズ情報。
※ここは、フレーム全体のバイト数（シリアライズされたオブジェクトのサイズ）を示しており、例えば 0x4900000000000008 という値かもしれません。
この値により、pickle は次にどれだけのバイトを読み込むべきか判断します。

残りのデータ
c05627472656594
→ この部分は、pickle の内部命令と、オブジェクトデータの一部です。
8c04506169729493
→ "8c" は短い文字列を表すオペコード（SHORT_BINUNICODE や BINUNICODE など）で、続くバイト列は文字列データに対応している可能性があります。
9429817d94288c03
→ ここも pickle の制御情報（MARK や TUPLE、DICT の開始など）や、オブジェクトの構造を示す命令が含まれていると考えられます。
6b657994430931000000000000000194
→ この部分は、"key"（6b6579）という文字列が含まれているのが確認できます。
ASCIIコードでそれぞれ "6b" → 'k', "65" → 'e', "79" → 'y'
「430931」は、ASCII で見ると "C091" のようにも見えますが、厳密にはバイナリデータとして扱われるため、単純に文字列として解釈できない部分
0000000000000001
→keyの値
94?
